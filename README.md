...
- [assets/js/analytics.js](assets/js/analytics.js): Google Analytics (measurement ID `G-67D1H8GX6X`), linked from every page. It only runs on the live site (`www.bdnix.com` and `bdnix.com`), so local previews and the tests don't send anything or load Google's script. It records page views only; files opened in the tools never leave the browser.

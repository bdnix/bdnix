...
- [mp4-to-mp3/index.html](mp4-to-mp3/index.html), [assets/css/audio.css](assets/css/audio.css), [assets/js/audio.js](assets/js/audio.js), [assets/js/audio-core.js](assets/js/audio-core.js): audio converter, served at `/mp4-to-mp3/`. The core file holds the channel mixing, the MP3 and WAV encoding, and pulling the audio track out of MP4 and MOV files (so a long phone video is never read into memory whole), kept separate so it can be unit tested.
...
The browser decodes the audio itself (at 44.1 kHz), and [lamejs](https://github.com/zhuker/lamejs) encodes the MP3, so nothing is uploaded. That means the formats it can read depend on the browser: a file whose audio the browser can't decode is marked on the list with a message. From MP4 and MOV files (what phones record) only the audio track is read, using the file's index, so the video itself never has to fit in memory. The decoded sound is still held in memory whole, so very long recordings (hours) may be too big for a phone.
